...
from jwt import decode, ExpiredSignatureError, InvalidTokenError  # Replace 'jwt' with the library you use for token management
...
# TO VIEW CHANNELS LIST - UPDATED
def channels(request):
    # Skip authentication validation to allow public access

    # Proceed with MongoDB retrieval
    db, collection = connect_to_mongodb('Channel', 'dashboard')
    if db is not None and collection is not None:
        # Fetch all channels, regardless of user_id
        channels = collection.find()  # Removed filtering by user_id
        if channels:
            channel_list = []
            public_channel = 0
            total_sensor = 0
            for channel in channels:
                ...
                channel_data = {
                    ...
                    'privacy': channel.get('privacy', ' ')  # Include privacy for public/private check
                }
...
#     # Proceed with MongoDB retrieval as before
#     db, collection = connect_to_mongodb('Channel', 'dashboard')
#     if db is not None and collection is not None:
#         channels = collection.find({"user_id": request.user_id})
#         if channels:
#             channel_list = []
#             public_channel = 0
#             total_sensor = 0
    ...
    if request.method == 'POST':
        ...
        if matching_sensors:
            collection_channel.update_one(
                filter_criteria,
                {'$set': {'API_KEY': API_KEY, 'allow_API': "permit"}}
            )
            ...
            print(f"Successfully added sensors to channel {channel_id}.")
            return redirect('view_channel_sensor', channel_id=channel_id)
        else:
...
# UNSET SENSOR - Clears the API key from the channel directly
@csrf_exempt
